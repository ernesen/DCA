...
createWebsphereVariableServer=/tmp/${remoteWebServerDir}/createWebsphereVariableServer.csv
...
function add_user {
	...
	if [ $PROFILE_TYPE == "dmgr" ] ; then
		$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createUser; createUser('$USERNAME', '$PASSWORD', '$roleName')"		
	fi
}
...
function createNodeGroup {
	groupName=$1
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createNodeGroup; createNodeGroup('$groupName')"
}

function addNodeGroupMember {
	groupName=$1
	nodeName=$2
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import addNodeGroupMember; addNodeGroupMember('$groupName','$nodeName')"
}

function createCoreGroup {
	
	coreGroupName=$1

	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createCoreGroup; createCoreGroup('$coreGroupName')"
}

function createTemplateCluster {
...
function modifyServerPort {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import modifyServerPort; modifyServerPort('$serverName', '$nodeName','$endPointName','$port')"
}

function setJVMPropertiesCluster {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import setJVMPropertiesCluster; setJVMPropertiesCluster( '$clusterName', '$initialHeapSize', '$maximumHeapSize', '$jvmClassPath')"
}

function setJVMPropertiesNode {
...
function modifyThreadPool {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import modifyThreadPool; modifyThreadPool('$clusterName','$propName','$threadMin','$threadMax','$inactivityTimeout')"
}

function createDynamicCluster {
	...
	serverTemplate=$5
	enableHA=$6
	numVerticalInstances=$7
	
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createDynamicCluster; createDynamicCluster('$clusterName', '$nodeGroup','$coreGroup','$nodeTemplate','$serverTemplate','$enableHA','$numVerticalInstances')"  
}
...
function createJAASAuthData {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createJAASAuthData; createJAASAuthData('$AliasName','$User','$Passwd')" 
}

function createXADataSource {
	clusterName=$1
	jdbcName=$2
	dataSourceName=$3
	jndiName=$4
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createXADataSource; createXADataSource('$clusterName', '$jdbcName', '$dataSourceName', '$jndiName', '$authenAlias', '$databaseName', '$serverName', '$portNumber','$scope')"

} 

function createXADataSourceOracle {
	clusterName=$1
	jdbcName=$2
	dataSourceName=$3
	jndiName=$4
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createXADataSourceOracle; createXADataSourceOracle('$clusterName', '$jdbcName', '$dataSourceName', '$jndiName', '$authenAlias', '$databaseURL', '$scope')"

}  
 

function setJVMLogCluster {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import setJVMLogCluster; setJVMLogCluster('$clusterName', '$streamType', '$streamProperty', $StreamValue)"
}

function setJVMLogNode {
...
function modifyTransactionLog {
	
	serverName=$1
	logDirectory=$2

	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import modifyTransactionLog; modifyTransactionLog('$serverName', '$logDirectory')"
}
...
function createEnvironmentEntries {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createEnvironmentEntries; createEnvironmentEntries('$clusterName', '$propertyName', '$propertyValue')"
}

function modifyProcessExecutionCluster {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import modifyProcessExecutionCluster; modifyProcessExecutionCluster('$clusterName', '$runAsUser', '$runAsGroup', '$umask')"
}

function modifyProcessExecutionNode {
...
function modifyCPCustomProperties {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import modifyCPCustomProperties; modifyCPCustomProperties('$scope', '$clusterName', '$jdbcName', '$dataSourceName', '$propertyName', '$propertyType', '$propertyValue', '$propertyDescription')"

} 

function modifyDataSourceProperty {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import modifyDataSourceProperty; modifyDataSourceProperty('$scope', '$clusterName', '$jdbcName', '$dataSourceName', '$minConnections', '$maxConnections', '$connectionTimeout', '$reapTime', '$unusedTimeout', '$agedTimeout', '$purgePolicy')"
...
function addHostAlias {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import addHostAlias; addHostAlias('$hostName', '$dnsHost', '$port')"
...
function createSIBus {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createSIBus; createSIBus('$SIBusName')" 
}
...
function addSIBusMember {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import addSIBusMember; addSIBusMember('$busName','$targetArgs','$JNDIName','$authAlias','$schemaName')" 
}

function createSIBForeignBus {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createSIBForeignBus; createSIBForeignBus('$busName','$fBusName','$messagingEngine','$MQLinkName','$senderChannel','$hostName' ,'$port' ,'$receiverChannel')" 
}
...
function createSIBDestination {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createSIBDestination; createSIBDestination('$busName','$desName','$fBusName','$qType','$cluster')" 
}
...
function createSIBJMSConnectionFactory {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createSIBJMSConnectionFactory; createSIBJMSConnectionFactory('$clusterName','$queueName','$jndiName','$busName')" 
}

function createWMQConnectionFactory {
	
	clusterName=$1
	queueName=$2
	jndiName=$3
	qmgrName=$4
	qmgrSvrconnChannel=$5
	connectionNameList=$6

	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createWMQConnectionFactory; createWMQConnectionFactory('$clusterName','$queueName','$jndiName','$qmgrName','$qmgrSvrconnChannel','$connectionNameList')" 
}

function modifyWMQConnectionFactory {
	...
	qmgrSvrconnChannel=$5
	connectionNameList=$6
	containerAuthAlias=$7
	maxConnections=$8
	minConnections=$9

	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import modifyWMQConnectionFactory; modifyWMQConnectionFactory('$clusterName','$queueName','$jndiName','$qmgrName','$qmgrSvrconnChannel','$connectionNameList','$containerAuthAlias' ,'$maxConnections' ,'$minConnections')" 
}

function createJMSQueue {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createJMSQueue; createJMSQueue('$clusterName','$jndiName','$deliveryMode','$busName' ,'$queueName' ,'$connectQueueName')" 
}

function createWMQQueue {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createWMQQueue; createWMQQueue('$clusterName','$wmqqName','$jndiName','$queueName','$ccsid','$useNativeEncoding')" 
}

function createSIBJMSActivationSpec {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createSIBJMSActivationSpec; createSIBJMSActivationSpec('$clusterName','$specName','$jndiName','$desJndiName' ,'$messageSelector' ,'$busName' ,'$subHome' ,'$maxBatchSize' ,'$maxConcurrency')" 
}

function createWMQActivationSpec {
	...
	desJndiName=$4
	qmgrName=$5
	qmgrSvrconnChannel=$6
	connectionNameList=$7
	rescanInterval=$8
	ccsid=$9
	failIfQuiescing=${10}
	maxPoolSize=${11}
	poolTimeout=${12}
	stopEndpointIfDeliveryFails=${13}
	messageRetention=${14}
	
	
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createWMQActivationSpec; createWMQActivationSpec('$clusterName','$specName','$jndiName','$desJndiName' ,'$qmgrName' ,'$qmgrSvrconnChannel' ,'$connectionNameList', '$rescanInterval','$ccsid','$failIfQuiescing','$maxPoolSize','$poolTimeout','$stopEndpointIfDeliveryFails','$messageRetention')" 
}

function modifyWMQActivationSpec {
	...
	echo "DCA clusterName" $clusterName
	echo "DCA specName" $specName	
	echo "DCA authAlias" $authAlias
	echo "DCA propName" $propName
	echo "DCA propType" $propType
	echo "DCA propDesc" $propDesc
	echo "DCA propValue" $propValue
	echo "DCA required" $required

	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import modifyWMQActivationSpec; modifyWMQActivationSpec('$clusterName','$specName','$authAlias' ,'$propName' ,'$propType' ,'$propDesc' ,'$propValue','$required')" 
}

function createWebsphereVariableServer {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createWebsphereVariableServer; createWebsphereVariableServer ('$nodeName','$serverName','$symbolicName', '$value')"

}

function enableCPALLLDAP {
...
function createGroup {
	...
	#$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import enableSecurity; enableSecurity()"
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createGroup; createGroup('$groupName')"
}

function mapUsersToAdminRole {
	
	userName=$1
	roleName=$2

	#$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import enableSecurity; enableSecurity()"
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import mapUsersToAdminRole; mapUsersToAdminRole('$userName','$roleName')"
}

function mapGroupsToAdminRole {
	
	groupName=$1
	roleName=$2

	#$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import enableSecurity; enableSecurity()"
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import mapGroupsToAdminRole; mapGroupsToAdminRole('$groupName','$roleName')"
}
...
function createWebServer {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createWebServer; createWebServer('$nodeName', '$webServerName', '$configDir', '$webPort', '$adminUserID' , '$adminPasswd')"
...
function configWebServer {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import configWebServer; configWebServer('$nodeName', '$webServerName', '$configDir')"

} 

function modifySIBDestination {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import modifySIBDestination; modifySIBDestination('$busName', '$destName', '$point1', '$point2')"

}

function createSIBContextInfo {
...
function createJVMCustomProperties {

	clusterName=$1
	propName=$2
	propValue=$3

	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createJVMCustomProperties; createJVMCustomProperties('$clusterName', '$propName', '$propValue')"
...
function createScheduler {
	...
	pollInterval=$3
	tablePrefix=$4
	datasourceJNDIName=$5
	datasourceAlias=$6
	jndiName=$7
	workManager=$8 

	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createScheduler; createScheduler('$clusterName', '$scheName', '$pollInterval', '$tablePrefix', '$datasourceJNDIName', '$datasourceAlias', '$jndiName', '$workManager')"
...
function createWorkManager {
	...
	numAlarmThreads=$5
	isGrowable=$6
	workReqQFullAction=$7
	workReqQSize=$8 

	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createWorkManager; createWorkManager('$clusterName', '$wmName', '$jndiName', '$maxThreads', '$numAlarmThreads', '$isGrowable', '$workReqQFullAction', '$workReqQSize')"

}

function createWCCustomProperties {

	clusterName=$1
	propName=$2
	propValue=$3

	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createWCCustomProperties; createWCCustomProperties('$clusterName', '$propName', '$propValue')"

}

function createASCustomProperties {
	...
	$WAS_PROFILE_ROOT/bin/wsadmin.sh -conntype JSR160RMI -port 9809 -user $WHV_USER -password $HTTPPASS -lang jython -c "from DCA_libraries import createASCustomProperties; createASCustomProperties('$clusterName', '$specName', '$propName' , '$propValue' , '$propType' , '$propDesc' , '$required')"
}
