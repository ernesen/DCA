...
print "$Id: wsadminlib.py,v 1.1.2.5 2012/09/21 15:51:40 adang Exp $"
...
def createServerInClusterWtem(clustername, nodename, servername, Tnodename, Tservername, sessionReplication = False):
    """Create a new server in a cluster, return its id.
    Turn on session replication if sessionReplication is True"""
    m = "createServerInClusterWtem:"
    ...
    if sessionReplication:
        sop(m,'Calling AdminTask.createClusterMember([-clusterName %s -memberConfig[-memberNode %s -memberName %s -memberWeight 2 -replicatorEntry true] -firstMember [-templateServerNode %s -templateServerName %s -nodeGroup DefaultNodeGroup -coreGroup DefaultCoreGroup]' % (clustername,nodename,servername,Tnodename,Tservername))
        AdminTask.createClusterMember('[-clusterName %s -memberConfig [-memberNode %s -memberName %s -memberWeight 2 -replicatorEntry true] -firstMember [-templateServerNode %s -templateServerName %s -nodeGroup DefaultNodeGroup -coreGroup DefaultCoreGroup]]' % (clustername,nodename,servername,Tnodename,Tservername)) 
    else:
        sop(m,'Calling AdminTask.createClusterMember([-clusterName %s -memberConfig[-memberNode %s -memberName %s -memberWeight 2 -replicatorEntry false] -firstMember [-templateServerNode %s -templateServerName %s -nodeGroup DefaultNodeGroup -coreGroup DefaultCoreGroup]' % (clustername,nodename,servername,Tnodename,Tservername))
        AdminTask.createClusterMember('[-clusterName %s -memberConfig [-memberNode %s -memberName %s -memberWeight 2 -replicatorEntry true] -firstMember [-templateServerNode %s -templateServerName %s -nodeGroup DefaultNodeGroup -coreGroup DefaultCoreGroup]]' % (clustername,nodename,servername,Tnodename,Tservername))
...
def stopCluster( clustername ):
    ...
    # Wait for it to stop
    maxwait = 600  # wait about 10 minutes at most
    count = 0
...
def getServerNameForAllServersbutwebandproxy ():
    ...
    for node in nodes:
        ...
        for server in servers:
            ...
            if serverType == 'APPLICATION_SERVER' or serverType == 'NODE_AGENT' or serverType == 'DEPLOYMENT_MANAGER':
                serverIDList.append( (nodeName, serverName) )
                sop(m, 'Added config ID for server %s on node %s to output list' % (serverName, nodeName))
            #endif
        #endfor
    #endfor
...
def getServerNameForAllAppServers ():
    """This functions returns the config ID, node name, and server name for all application servers
       in the cell.
...
def addPreferredCoordinatorToCoreGroup( servername, coregroupname, nodename=None):
    """Adds the specified server to the specified coregroup."""
    ...
    if nodename:
        ...
        coregroupserver_id = getCoreGroupServerIdByServerNameandNodeName(coregroup_id, servername, nodename)
    else:
        #Edited it to accomedate same server name of different Nodes
...
def getCoreGroupServerIdByServerNameandNodeName( coregroup_id, servername, nodename ):
    """Returns the ID of a CoreGroupServer object representing the supplied server name."""
    m = "getCoreGroupServerIdByName:"
    #sop(m,"Entry. coregroup_id=%s servername=%s" % ( coregroup_id, servername ))
    coregroupserver_id = None
    ...
    if len(cgs_string) > 2:
        ...
        for cgs_id in cgs_list:
            cgs_name = getNameFromId(cgs_id)
            nodeName = AdminConfig.showAttribute(cgs_id, "nodeName")
            if servername == cgs_name and nodeName == nodename:
                #sop(m,"Found match. cgs_id=%s cgs_name=%s servername=%s" % ( cgs_id, cgs_name, servername ))
                coregroupserver_id = cgs_id
                break
            # else:
...
def setDeploymentAutoStart(deploymentname, enabled, deploymenttargetname=None):
    ...
    if (len(deployments) > 0) :
        ...
        # Beware, rawTargetMappings is returned as one python string which contains square
        # brackets that make it look like a python list, but it is really just one string.
        sop(m,"rawTargetMappings=%s" % ( repr(rawTargetMappings) ))
        ...
        # ['(cells/ding6DmgrCell/applications/commsvc.ear/deployments/commsvc|deployment.xml#DeploymentTargetMapping_1253647752484)',
        #  '(cells/ding6DmgrCell/applications/commsvc.ear/deployments/commsvc|deployment.xml#DeploymentTargetMapping_1253647806679)']
        ...
        # Beware, rawDeploymentTargets is returned as one python string which contains square
        # brackets that make it look like a python list, but it is really just one string.
        sop(m,"rawDeploymentTargets=%s" % ( repr(rawDeploymentTargets) ))                       # DBG
        # Convert the single string to a real python list containing strings.
        ...
        for deploymentTarget in deploymentTargetList:
            sop(m,"deploymentTarget=%s" % ( deploymentTarget ))
            current_deployment_target_name = getNameFromId(deploymentTarget)
            current_deployment_target_number = getNumberFromId(deploymentTarget)
            sop(m,"current_deployment_target_name=%s current_deployment_target_number=%s" % ( current_deployment_target_name, current_deployment_target_number ))
            # Set autostart on either all deployments or the specified deployment.
            if None == deploymenttargetname or current_deployment_target_name == deploymenttargetname:
                sop(m,"Setting autostart on deployment. deploymentTarget=%s" % ( deploymentTarget ))

                # Find the associated targetMapping and set enabled/disabled.
                for targetMapping in targetMappingList:
                    sop(m,"targetMapping=%s" % ( targetMapping ))
                    target_mapping_number = getNumberFromId(targetMapping)
                    if target_mapping_number == current_deployment_target_number:
                        valueString = '[[enable "%s"]]' % ( enabled )
                        sop(m,"Calling AdminConfig.modify with id=%s and value=%s" % ( targetMapping, valueString ))
                        AdminConfig.modify(targetMapping, valueString)
                    else:
                        sop(m,"no match.")
            else:
                sop(m,"Skipping deployment. deploymenttargetname=%s current_deployment_target_name=%s" % ( deploymenttargetname, current_deployment_target_name ))
    else:
...
def ensureHostAlias( virtualhostname, aliashostname, port ):
    ...
    if (virtualhostname, aliashostname, port):
        print "In Here"
...
def deleteHostAliasHost( virtualhostname, aliashostname, port='80' ):
    """Deletes host alias"""
    m = "deleteHostAliasHost:"
    #sop(m,"Entry. virtualhostname=%s aliashostname=%s port=%s" % ( repr(virtualhostname), repr(aliashostname), repr(port) ))
    # Prevent dumb errors.
        ...
        if aliases != None:
            ...
            for alias_id in aliases:
                ...
                if alias_id != None and alias_id != '':
                    # Alias is a HostAlias object
					sop(m,"Before Delete host=%s and aliashost=%s" % ( repr(aliases),aliashostname))
...
def setServerPMI(nodename, servername, initialSpecLevel, enable):
...
def getTuningParams(nodename, servername):
    server_id = getServerByNodeAndName( nodename, servername )
    return AdminConfig.list('TuningParams', server_id)
...
def modifyTuningParamsProp(nodename,servername,name,value):
    tc = getTuningParams(nodename,servername)
    attrs = []
    attrs.append( [ name, value ] )
    return modify(tc, attrs)
...
############################################################
# misc methods
def getFritzComponent(testpath):
    """ Gets the name of the Fritz component (i.e PXY,SIP) """
    array = testpath.split('/')
    return array[-3]
...
def getSopTimestamp():
    ...
    # Assemble the formatting string in pieces, so that CMVC does not interpret
...
def createSIBus(SIBusName, desc, busSecurity="", clusterName=None, nodeName=None, serverName=None, scope=None):
    ...
    if((len(bus) == 1) or (len(bus) == 0)):
        params = ["-bus", SIBusName, "-description", desc]
        if(not(busSecurity == "")):
...
def createSIBus_ext(SIBusName, busSecurity, interAuth, medAuth, protocol, discard, confReload, msgThreshold ):
    """ This method encapsulates the actions needed to create a bus on the current cell.
    ...
    bus = AdminConfig.getid('/SIBus:%s' % SIBusName)
    if((len(bus) == 1) or (len(bus) == 0)):
        params = ["-bus", SIBusName, "-interEngineAuthAlias", interAuth, "-mediationsAuthAlias", medAuth, "-protocol", protocol, "-discardOnDelete", discard, "-configurationReloadEnabled", confReload, "-highMessageThreshold", msgThreshold ]
        if(not(busSecurity == "")):
            params.append("-busSecurity")
            params.append(busSecurity)
        else:
...
def addSIBusMember_ext (clusterName, nodeName, serverName, SIBusName, messageStoreType, logSize, logDir, minPermStoreSize, minTempStoreSize, maxPermStoreSize, maxTempStoreSize, unlimPermStoreSize, unlimTempStoreSize, permStoreDirName, tempStoreDirName, createDataSrc, createTables, authAlias, schemaName, dataSrcJNDIName ):
    ...
    """
    m = "addSIBusMember: "
    sop (m, "Entering addSIBusMember EXT function...")
...
def createSIBJMSConnectionFactory(clusterName, serverName, jmsCFName, jmsCFJNDI, jmsCFDesc, jmsCFType, SIBusName, scope, authAlias="", provEndPoints=""):
...
def createSIBJMSQueue(jmsQName, jmsQJNDI, jmsQDesc, SIBQName, busName, scope):
    ...
    params = ["-name", jmsQName, "-jndiName", jmsQJNDI, "-description", jmsQDesc, "-queueName", SIBQName, "-busName", busName]
    AdminTask.createSIBJMSQueue(scope, params)
#endDef

def createSIBQueue(clusterName, nodeName, serverName, SIBQName, SIBusName, SIBQDescription=None):
    ...
    #--------------------------------------------------------------------
    if(clusterName == "none"):
        params = ["-bus", SIBusName, "-name", SIBQName, "-description", SIBQDescription, "-type", "Queue", "-node", nodeName, "-server", serverName]
    else:
        params = ["-bus", SIBusName, "-name", SIBQName, "-description", SIBQDescription, "-type", "Queue", "-cluster", clusterName]
    #endElse
...
def createSIBJMSActivationSpec(activationSpecName, activationSpecJNDI, jmsQJNDI, destinationType, authAlias, maxBatchSize, maxConcurrency, SIBusName, scope, messageSelector = None):
    ...
    if(not(authAlias == None)):
        params.append("-authenticationAlias")
        params.append(authAlias)
    #endIf
    if(not(messageSelector == None)):
        params.append("-messageSelector")
        params.append(messageSelector)
    #endIf
    params.append("-maxBatchSize")
    params.append(int(maxBatchSize))
    params.append("-maxConcurrency")
    params.append(int(maxConcurrency))
...
def createJAAS(auth_alias, auth_username, auth_password, auth_desc):
    """ This method encapsulates the actions needed to create a J2C authentication data entry.
        Add parameter auth_desc - Alain Dang April 19, 2012
    ...
    Parameters:
        ...
        auth_desc - Alias description in authentication entry in String format
    Returns:
        No return value
    """
    #---------------------------------------------------------
    ...
    desc_attr = ["description", auth_desc]
...
def createDataSource_ext ( jdbcProvider, datasourceName, datasourceDescription, datasourceJNDIName, statementCacheSize, authAliasName, datasourceHelperClassname, dbType, nonTransDS='', cmpDatasource='true', databaseName=None, serverName=None, portNumber=None, driverType=None, URL=None, informixLockModeWait=None, ifxIFXHOST=None, scope=None, clusterName=None, nodeName=None, serverName_scope=None, xaRecoveryAuthAlias=None ):
    """Creates a DataSource under the given JDBCProvider; removes existing objects with the same jndiName"""
    ...
    attrs.append( [ 'authDataAlias', authAliasName ] )
    attrs.append( [ 'datasourceHelperClassname', datasourceHelperClassname ] )
#    attrs.append( [ 'containerManagedPersistence', cmpDatasource ] )
#    attrs.append( [ 'componentManagedAuthenticationAlias', authAliasName ] )
#    attrs.append( [ 'mapping', mapping ] )

    sop (m, "jdbcProvider = %s" % jdbcProvider)
    ...
    datasourceID = removeAndCreate( 'DataSource', jdbcProvider, attrs, ['jndiName'])
    sop (m, "calling Create to create datasource")
    create('J2EEResourcePropertySet', datasourceID, [], 'propertySet')
...
def createDerbyDataSource( jdbcProvider, datasourceJNDIName, authAliasName, databaseName ):
